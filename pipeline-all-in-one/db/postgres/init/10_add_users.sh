...
# The usernames are defined by $PREFILTER_USER, $CONNECT_USER and $INGESTION_USER.
# The passwords are loaded from files defined by $PREFILTER_PASSWORD_FILE, $CONNECT_PASSWORD_FILE and $INGESTION_PASSWORD_FILE.
...
if [ ! -f "$PREFILTER_PASSWORD_FILE" ] || [ ! -f "$CONNECT_PASSWORD_FILE" ] || [ ! -f "$INGESTION_PASSWORD_FILE" ]; then
