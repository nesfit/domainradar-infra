...
(
    ...
    details     JSONB            NULL,
    CONSTRAINT Classification_Category_Result_Unique UNIQUE (domain_id, timestamp, category)
);
...
-- Create a dummy target table for the collection results Kafka Connect sink
...
-- Create a dummy target table for the classification results Kafka Connect sink
...
BEGIN
    ...
    INSERT INTO Collection_Result (domain_id, ip_id, source, status_code, error, timestamp, raw_data)
...
BEGIN
    ...
    FOR v_classification_result IN SELECT value FROM jsonb_array_elements(v_classification_results) AS t(value)
        LOOP
            ...
            INSERT INTO Classification_Category_Result (domain_id, timestamp, category, probability, description, details)
            ...
            FOR v_detail_rec IN SELECT key, value FROM jsonb_each(v_details)
                LOOP
                    ...
                    INSERT INTO Classifier_Output (result_id, classifier, probability, additional_info)
                    VALUES (v_result_id,
                            v_detail_rec.key::SMALLINT,
                            v_detail_rec.value::DOUBLE PRECISION,
                            NULL)
                    ON CONFLICT (result_id, classifier) DO UPDATE
                        SET probability     = EXCLUDED.probability,
